...
| `02-engine-sim` 引擎单元 & 长时模拟 | 22 | 开局、行动点、修炼、突破、战斗终止、装备、存档往返、丹药、年份推进；**100 局随机全流程模拟**（不变量 + 无 NaN）+ 数值平衡抽样；**锻体系统**（未解锁拒绝/消耗收益/每大境界10次上限/跨境界重置） |
...
| 02 引擎单元测试 & 长时模拟 | 22 | 0 | 0 |
| 03 UI / DOM 层（含弹窗/锻体回归） | 10 | 0 | 0 |
| **合计** | **48** | **0** | **0** |

> 结果：**全量 48 用例通过，0 风险**。详见 `reports/test-report-*.md` 最新一份。
