...
c += `probe=(function(){
  ...
  fail('STAGES �?2�?, STAGES.length === 12);
  fail('末阶=元婴后期', STAGES[11].realm === '元婴' && STAGES[11].sub === '后期' && STAGES[11].bigRealm === 3);
  fail('无第12�?化神已砍)', STAGES[12] === undefined);
  fail('NEED 末�?19000', NEED[11] === 19000);
  fail('BIG_REALMS=4�?, BIG_REALMS.length === 4 && BIG_REALMS[3] === '元婴');
  fail('天劫名单无化�?, TRIBULATIONS.join() === '金丹,元婴,飞升');
  fail('开局自带吐纳诀', s.techs.join() === 'tunai');
  fail('REALM_META 无化�?, !REALM_META['化神']);
  ...
  fail('修炼公式(60+悟性x20)x灵根x功法(吐纳1.2): �? + gain + ' 期望' + exp0, gain === exp0);
  ...
  FR.v = null;
  fail('炼气中期突破(无功�?', rb.win && !rb.tech && s.idx === 1);
  s.qi = 1e9;
  FR.v = 0.01;
  rb = E.breakthrough(s);
  FR.v = null;
  fail('炼气后期突破(无功�?', rb.win && !rb.tech && s.idx === 2);
  ...
  fail('筑基突破获功�?, !!rb.tech && s.techs.indexOf(rb.tech) >= 0);
  out.push('  筑基功法=' + (rb.tech ? TECHNIQUES[rb.tech].name : '�?));

  // 筑基�?后：无功�?  s.qi = 1e9;
  FR.v = 0.01;
  rb = E.breakthrough(s);
  FR.v = null;
  fail('筑基中期小突�?无功�?', rb.win && !rb.tech && s.idx === 4);
  s.qi = 1e9;
  FR.v = 0.01;
  rb = E.breakthrough(s);
  FR.v = null;
  fail('筑基后期小突�?无功�?', rb.win && !rb.tech && s.idx === 5);
  ...
  fail('飞升获仙阶功�?, !!rb.tech);
  out.push('  飞升功法=' + (rb.tech ? TECHNIQUES[rb.tech].name : '�?));

  // 怪物池：元婴�?bi 上限钳制
  const s2 = E.startLife('�?); E.commitStart(s2, 'dujie');
  s2.idx = 11; s2.realm = '元婴';
  FR.v = 0.01;
  const m1 = E.enemyGen(s2, 'mob', 3);
  FR.v = null;
  const yuanyingNames = ['玄冥�?,'血瞳魔�?,'九幽魂主','天罡石灵','远古荒鲲','堕落真仙'];
  fail('怪物池钳制在元婴�?, yuanyingNames.indexOf(m1.name) >= 0);
  out.push('  怪物=' + m1.name);
  ...
  s3.actionsLeft = 1;
    let ev1 = E.social(s3);
  fail('social -> multi 3 events from ������(shejiao)', ev1 && ev1.multi === true && ev1.events.length >= 1 && ev1.events.length <= 3 && ev1.events.every(function (e) { return EVENTS.shejiao.some(function (x) { return x.id === e.id; }); }));
  out.push('  no-sect events=' + ev1.events.length);
  s3.actionsLeft = 1;
  s3.sect = 'qingyunjian';
  ev1 = E.social(s3);
  fail('sect social -> still from ������(shejiao), not SECT_SOCIAL', ev1 && ev1.multi === true && ev1.events.every(function (e) { return EVENTS.shejiao.some(function (x) { return x.id === e.id; }); }));
  ...
  fail('宗门年末池按大境界过�?, poolOK.every(function(e){ return e.min <= bi && e.max >= bi; }));
  ...
  // 战斗掉落支持 atk/hpMax
  const sp = { name: '测试�?, atk: 3, hp: 30, loot: { stone: 30, atk: 2, hpMax: 20 } };
