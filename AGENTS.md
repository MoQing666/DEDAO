...
```bash
node test/automated/run.js     # 依次跑 01~13，当前 262/262 全过
```
...
| 文件 | 例数 | 覆盖 |
|---|---:|---|
| `01-static-data.test.js` | 26 | 静态数据一致性 & 引用完整性（含宗门商品单货币结构、法宝文案「++」守卫、六维面板文案守卫、**事件 effect 键 ⊆ applyOps 白名单**守卫、**手机端滚动适配在位性**守卫、**AGENTS.md 模块表计数对账**守卫、**DEDAO_项目简介.md 对外口径对账**守卫、**已删字段「突破次数」不得复活**守卫） |
...
- **删了什么**（`js/` 侧 6 处）：
  ...
  | `ui.js` `statSheet` / `diffLines` / 结算页 | 快照字段 + 「生涯突破 X→Y 次」+「一生突破 N 次」 | 删 |
