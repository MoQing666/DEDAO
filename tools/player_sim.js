 ...
 *
 * 仍保留、但已非「公式副本」的两块：
 *   · combatSim —— 蒙特卡洛对战抽样模型（减伤流程与引擎同序：护盾→百分比→绝对→除算），
 *     本就不是某个引擎函数的镜像，属模拟模型本身。
 *   · advEnemy / deathEnemyDynamic —— 仅做「参数适配」：把模拟器自己的
 *     (boss, elite, jie) 约定换算成 enemyStats 的入参，再调 Engine.enemyStats。
 *     它们不重写任何数值公式，故不算分叉风险；原先里面的 genEnemyStats 已删除、
 *     直接指向 Engine.enemyStats（见第 6 节）。
...
// 敌人基线模型：直接复用引擎的 enemyStats（2026-09-15「第四大 bug 类」收尾 + 本处技术债清理）
//   原先这里是 engine.js enemyStats 的字节级手抄副本；现改为指向 Engine.enemyStats，
//   删除镜像、统一口径——引擎改公式，模拟器自动跟随，不再有分叉风险。
const ADVENTURE_GRADE = { huang: 0, xuan: 1, di: 2, tian: 3, xian: 3 };
const genEnemyStats = ENG.enemyStats;
...
function deathEnemyDynamic(p, idx, jieDiff) {
  ...
  // 直接复用引擎 enemyStats（固定境界基准 × 递增系数 × 叠劫），不再手抄同一套数学
  return ENG.enemyStats(DEATH_IDX_REALM[idx], sc.atkMul, sc.hpMul, jieDiff);
}
