...
| 路径 | 说明 |
|---|---|
| `test/automated/run.js` + `01~14-*.test.js` | 自动套件（271 例），新套件须登记 `MODULES` |
...
```bash
node test/automated/run.js     # 依次跑 01~14，当前 271/271 全过
```
...
| 文件 | 例数 | 覆盖 |
|---|---:|---|
| `01-static-data.test.js` | 28 | 静态数据一致性 & 引用完整性（含宗门商品单货币结构、法宝文案「++」守卫、六维面板文案守卫、**事件 effect 键 ⊆ applyOps 白名单**守卫、**手机端滚动适配在位性**守卫、**AGENTS.md 模块表计数对账**守卫、**DEDAO_项目简介.md 对外口径对账**守卫、**已删字段「突破次数」不得复活**守卫、**法术大表 dmg/cost 与 data.js 逐条对账**守卫、**好感度星星空心/金色两处渲染一致**守卫） |
...
- **附带**：`node tools/pool-audit.js` 0 错误 0 警告（装备 58 / 法宝 47）。
