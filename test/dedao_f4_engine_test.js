...
c += `probe=(function(){
  ...
  E.saveState(s, 0);
  fail('saveState(slot0) 后 slotExists(0)=true', E.slotExists(0) === true);
  ...
  fail('loadState(2) 回读一致', sBack && sBack.name === '存档测试' && sBack.idx === 0);
  fail('存档2镜像写入自动档', E.slotExists(null) === true && !!localStorage.getItem('dedao_save'));
  ...
  E.commitStart(s2, 'liancai');
  s2.techs = ['tunai', 'shengong', 'yuhuo', 'hanshuang', 'leiyin', 'suodi'];
  delete s2.techEquip;
  E.saveState(s2, 1);
  localStorage.removeItem('dedao_save');
  const mig = E.loadState(1);
  fail('旧档迁移出 techEquip', !!mig.techEquip && !!mig.techEquip.xinfa && Array.isArray(mig.techEquip.shufa));
  fail('迁移心法=最高倍率(吐纳1.2 vs 玄天1.25 -> shengong)', mig.techEquip.xinfa === 'shengong');
  const ap0 = E.actionPoints(mig);
  fail('迁移法术位=actionPoints(' + ap0 + ')', mig.techEquip.shufa.length === Math.min(3, ap0));
  fail('迁移遁术=最优', mig.techEquip.dunshu === 'suodi');

  // ---- 法术位上限与切换 ----
  E.clearState();
  const s3 = E.startLife('法术测试');
  E.commitStart(s3, 'liancai');
  ...
  fail('装备满 ' + ap3 + ' 门法术', s3.techEquip.shufa.length === ap3);
  const first = s3.techEquip.shufa[0];
  fail('满位后装备被拒(false)', E.toggleShufa(s3, 'jianqi') === false);
  ...
  fail('equippedShufa 排好序', E.equippedShufa(s3).length === ap3);

  // ---- 战斗法术（仅装备之法术入列；显式释放一次） ----
  const s4 = E.startLife('战斗测试');
  E.commitStart(s4, 'liancai');
  ...
  fail('显式法术出招包含法术名', r1.lines.join().indexOf(TECHNIQUES.yuhuo.name) >= 0);
  fail('法术误伤敌人(伤害>0或描述)', s4.battle.hp < hp0 && r1.lines.join().indexOf('伤害') >= 0);
  fail('施法消耗灵力', s4.mo === mo0 - (TECHNIQUES.yuhuo.cost || 15));
  const r2 = E.combatAct(s4, 'spell', 'yuhuo');
  fail('法术第二次施放或分胜负(灵力制)', r2.done === true || s4.mo < mo0 - (TECHNIQUES.yuhuo.cost || 15));
  const s4b = E.startLife('战斗测试2');
  E.commitStart(s4b, 'liancai');
  ...
  fail('灵力不足禁施法', r2b.lines.join().indexOf('灵力不足') >= 0);
  fail('灵力不足时灵力不减', s4b.mo === 0);
  ...
  fail('30层中至少抽中1次灵铁节点', ironHits > 0);
  ...
  E.commitStart(te, 'liancai'); // 炼气 bigIdx0
  const tiersSeen = {};
  for (let k = 0; k < 400; k++) {
  ...
  fail('同年二次被拒', r2y.ok === false && r2y.msg.indexOf('一年') >= 0 && sy.actionsLeft > 5);
  sy.year = 2;
  const r3y = E.startAdventure(sy);
  fail('次年可再入', r3y.ok === true);

  // ---- 灵力（mo）系统 ----
  const sm = E.startLife('灵力测试');
  E.commitStart(sm, 'liancai');
  const moA = sm.mo, moMaxA = sm.moMax;
  fail('初始灵力与上限', moA > 0 && moA <= moMaxA);
  sm.linggen = LINGGEN_POOL.filter(function (x) { return x.id === 'bing'; })[0];
  E.refreshStats(sm);
  fail('冰灵根灵力上限+30', sm.moMax === moMaxA + 30);
  const mm2 = E.calcMoMax(sm);
  sm.mo = 1;
  E.moGain(sm, 1);
  fail('moGain 恢复且封顶', sm.mo === sm.moMax && E.calcMoMax(sm) === mm2);
  sm.mo = 5;
  ...
  fail('炼气期未圆满 breakInfo 无天劫', tb0.trib === null);
  stb.qi = 1e9;
  ...
  fail('金丹前 breakInfo 劫=金丹', tb2.trib === '金丹' && tb2.mode === 'trib');
  const tb3 = E.breakthrough(stb);
  fail('突破要求先渡劫(needTrib)', tb3.needTrib === true && stb.trib && stb.trib.target === '金丹' && stb.trib.ren === false);
  const tb3b = E.breakthrough(stb);
  fail('未渡人劫再点仍被拦', tb3b.needTrib === true);
  ...
  fail('天劫化身数值与禁逃', tjS.atk >= 20 && tjS.hp > tjS.atk && tjS.noFlee === true && tjS.dujie === true);
  ...
  if (won.win) {
    ...
    fail('败于天劫 -> 损修为清劫', dff.win === false && (dff.died || stb.qi < 1e9) && stb.trib === null);
  }
  E.clearState();
  const stb2 = E.startLife('渡劫失败');
  E.commitStart(stb2, 'liancai');
  stb2.qi = 1e9;
  E.breakthrough(stb2);
  ...
  stb2.trib.ren = true;
    ...
    fail('渡劫失败损修为', df.win === false && (df.died || stb2.qi <= 7e8) && stb2.trib === null);

  // ---- 修仙百艺：灵田 / 灵矿 ----
  const sf = E.startLife('百艺测试');
  E.commitStart(sf, 'liancai');
  sf.actionsLeft = 10;
  sf.herb = 20;
  const pf = E.plantField(sf, 'lingshen');
  fail('播种成功扣草', pf.indexOf('灵草田') >= 0 && sf.herb === 19);
  const fi = E.fieldInfo(sf, 0);
  fail('灵田信息(未熟)', fi && fi.done === false && fi.name === '灵草田');
  sf.year += 2;
  ...
  fail('金丹遁速=3', sd.dunSpeed === 3 && sd.realm.indexOf('金丹') >= 0);

  // ---- 修为满提示 ----
  const sc = E.startLife('修为满测试');
  E.commitStart(sc, 'liancai');
  sc.qi = Engine.requireNeed(sc);
  const cr = E.cultivate(sc);
  fail('修为满拒绝修炼', cr.indexOf('修为已满') >= 0);

  // ---- 宗门活动：降妖除魔 ----
  const sx = E.startLife('降妖测试');
  E.commitStart(sx, 'liancai');
  sx.sect = 'qingyunjian';
  sx.actionsLeft = 5;
  ...
  fail('元婴中期breakInfo劫=飞升', bi10.trib === '飞升');
