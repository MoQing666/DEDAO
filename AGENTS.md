...
> 最后更新：2026-09-13（`/init` 刷新）。本文档是项目的事实来源（source of truth），每次大规模改动后必须刷新。
...
46. **仙命（金阶轮回命格）10 条重写 + 图鉴「仙命」卷 + 秘境掉率文案收敛与四调 + 同名装备凭空消失 BUG + 负体魄地板（2026-09-14，用户逐字给定数值）**：
    ...
    - 缓存 **v126/dedao-v164**。测试 **235/235**（主仓库 + `dist/DEDAO_release` + `dist/taptap/dedao` 三跑一致），`tools/pool-audit.js` 0 错 0 警。
