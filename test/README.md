...
| `02-engine-sim` 引擎单元 & 长时模拟 | 19 | 开局、行动点、修炼、突破、战斗终止、装备、存档往返、丹药、年份推进；**100 局随机全流程模拟**（不变量 + 无 NaN）+ 数值平衡抽样；**锻体系统**（未解锁拒绝/消耗收益/每大境界10次上限/跨境界重置） |
| `03-ui` UI / DOM 层（jsdom） | 10 | 页面加载无报错、标题页、开局流程（轮回→起名→主界面）、行动按钮点击、底部弹窗、轮回塔、localStorage 存档、自定义弹窗回归、**锻体页（未解锁引导/解锁后进入与淬炼计数）** |
...
| 02 引擎单元测试 & 长时模拟 | 19 | 0 | 0 |
| 03 UI / DOM 层（含弹窗/锻体回归） | 10 | 0 | 0 |
| **合计** | **45** | **0** | **0** |

> 结果：**全量 45 用例通过，0 风险**。详见 `reports/test-report-*.md` 最新一份。
