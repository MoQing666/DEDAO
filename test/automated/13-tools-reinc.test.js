...
module.exports = async function build() {
  ...
  S.case('tools/ 不得再手抄引擎公式/数据表（已知陈旧写法黑名单 + 必须走 _engine_loader）', (t) => {
    ...
    const FORBID = [
      ...
      // 2026-09-15 实锤：法术耗蓝手抄成 黄15/玄25/地45/天70，真值 40/65/115/175（低估约 2.5 倍）。
      // dmg 旧值恰好等于各品级上限所以看不出来，cost 却让「可持续施放次数」翻倍，
      // 基于它得出的「耗蓝够用、无需下调」结论整个失效。现已改为从 TECHNIQUES 推导。
      [/SPELL_COST\s*=\s*\{[^}]*黄:\s*15/, '过期的法术耗蓝表（黄15/玄25/地45/天70，真值 40/65/115/175）；须从 `TECHNIQUES` 推导'],
      // 秘境敌人 v3 旧公式（挂钩玩家攻/血）已被引擎判定为设计失误并移除，
      // 现为「固定境界基数 ENEMY_REALM_BASE × 深度系数」。
      [/hits\s*=\s*\(\s*elite\s*\?\s*5\.0/, '残留秘境 v3 旧公式（hits=4+depth×0.5，挂钩玩家攻/血，引擎已移除）'],
    ];
