...
> 最后更新：2026-09-15（变更日志 #57）。本文档是项目的事实来源（source of truth），每次大规模改动后必须刷新。
...
```bash
node test/automated/run.js     # 依次跑 01~12，当前 258/258 全过
```
...
| 文件 | 例数 | 覆盖 |
|---|---:|---|
| `01-static-data.test.js` | 24 | 静态数据一致性 & 引用完整性（含宗门商品单货币结构、法宝文案「++」守卫、六维面板文案守卫、**事件 effect 键 ⊆ applyOps 白名单**守卫、**手机端滚动适配在位性**守卫、**AGENTS.md 模块表计数对账**守卫） |
