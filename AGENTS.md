...
> 最后更新：2026-09-10。本文档是项目的事实来源（source of truth），每次大规模改动后必须刷新。
...
- **代码规模（2026-09-10 统计）**：JS 共 **14169 行**（`audio.js` 353 / `data.js` 3370 / `engine.js` 3690 / `ui.js` 6753，另含文件尾 3 行），其中**有效代码 12919 行**（剔除空行与纯注释行 1250 行）；`index.html` 541 行、`css/style.css` 1176 行、`sw.js` 50 行。
  文件体积：`data.js` ~230KB / `engine.js` ~173KB / `ui.js` ~314KB / `style.css` ~49KB。
...
14. **软著材料行数口径刷新（2026-09-10）**：按 v84 源码重算——JS 共 **14169 行**（有效代码 **12919 行**，剔除空行/纯注释 1250 行）；重生成 `软著源程序_得道飞升模拟器.pdf`（**60 页 × 每页 50 行有效代码**）。同步刷新 `软著申请材料清单.md`、`软著申请表_逐字段填写速查.md`、`tools/gen_manual_pdf.py`、`tools/gen_manual_docx.py` 中的源程序量（14125→14169）与版本号（v81→v84），并重出说明书 PDF/DOCX。**口径**：源程序量以「含空行注释的物理行数」填报（14169），与 `tools/gen_softcopy_pdf.py` 输出的 `原始总行数` 一致。
