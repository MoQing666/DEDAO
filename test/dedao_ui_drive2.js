...
async function run() {
  ...
  if ($('name-input')) $('name-input').value = 'é’å´–';
  ...
  for (let layer = 1; layer <= 8; layer++) {
    ...
    await sleep(10);
    // result text 'ç»§ç»­' first (chapterAppend), then battle may open
    await clickUntilClosed('chapter-actions', 6);
    // battle?
    const battle = $('battle');
    if (battle.style.display === 'flex') {
      ...
      if (layer === 1) {
        ...
        console.log('AIä»£æ‰“ result -> chapter-title now: [' + $('chapter-title').textContent + ']');
      } else {
